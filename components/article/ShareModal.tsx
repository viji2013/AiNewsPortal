...
export function ShareModal({ isOpen, onClose, article }: ShareModalProps) {
  ...
  const shareText = `${article.title} - AI News`
  ...
  const handleRedditShare = () => {
    const redditUrl = `https://reddit.com/submit?url=${encodeURIComponent(articleUrl)}&title=${encodeURIComponent(shareText)}`
    ...
    >
      <div className="space-y-3">
        ...
        <button
          ...
          className="w-full flex items-center gap-3 px-4 py-3 rounded-lg border border-slate-700 bg-slate-800/50 hover:bg-slate-700/50 transition-colors"
        >
        ...
        >
          ...
          <span className="text-white font-medium">Share on Reddit</span>
        </button>
